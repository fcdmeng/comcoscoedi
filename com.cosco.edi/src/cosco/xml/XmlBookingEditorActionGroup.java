...
//tableViewer.getElementAt(table.getSelectionIndex());
...
public class XmlBookingEditorActionGroup extends EditorActionGroup {
	TableViewer tv;
	private Action selectVoyage = new SelectVoyage();

	private ConsoleCommand logconsole = new ConsoleCommand();

	public XmlBookingEditorActionGroup(TableViewer tv) {
		super(tv);
		this.getDownloadDataAction().setEnabled(false);

		// this.fillContextMenu(new MenuManager());
		// hookContextMenu();
		Menu menu = new Menu(tv.getControl());
		new MemuRight(menu, this.getTv().getTable());
	...
	private class SelectVoyage extends Action {
		public SelectVoyage() {
			...
			setHoverImageDescriptor(ImagesContext
					.getImageDescriptor(ImagesContext.SELECTVOYAGE));
		}
	...
	public void fireFirstAction() {
		...
		final String vslkey_seq = Context.getInstance().getBasvslvoy()
				.getSequence();

		tv = getTv();
		tv.setInput(new XmlbookingDAO().getXmlbooking(
				" SELECT * FROM XMLBOOKING WHERE VSLVOYKEY = 13492", true));
		refreshActionsState();
	}
	...
	public void fireDownloadDataAction() {
		...
		Job job = new Job("读取数据……") {
			...
			protected IStatus run(IProgressMonitor monitor) {
				...
				clientData = new EcdbookingDAO()
						.getEcdbooking("select * from Ecdbooking where vslkey_seq='"
								+ vslkey_seq + "'");
				...
				serverData = new EcdbookingDAO().getEcdbooking(
						"select * from Ecdbooking where vslvoykey="
								+ vslkey_seq + " ", true);
				...
				for (int i = 0; i < serverData.size(); i++) {
					...
					if (new EcdbookingDAO().inserEcdbooking(o) == true) {
						monitor.setTaskName("提单号:" + o.getBlno() + "……插入数据成功!"
								+ "[" + i + "]");
					} else {
